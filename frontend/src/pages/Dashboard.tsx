...
export default function Dashboard() {
  ...
  const handleCreateBot = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', niche: '', prompt: '', knowledge_base: '', style: 'Professional', greeting: 'Salam, necə kömək edə bilərəm?', discount_rules: '', human_handoff: '' });
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-10 max-w-7xl mx-auto pt-24 lg:pt-10 min-h-screen pb-20">
      ...
      {/* Toast Messages */}
      <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[1000000] w-full max-w-md px-4 space-y-3 pointer-events-none">
        {successMsg && (
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="flex-1 sm:flex-none px-8 py-3 bg-indigo-600 text-white rounded-[1.25rem] font-black text-xs flex items-center justify-center gap-2 shadow-xl shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all uppercase tracking-widest"
          >
            Yeni Bot
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-50 bg-slate-50/30 font-black text-slate-800 flex items-center gap-2">
               <Activity size={18} className="text-indigo-600" /> Son Aktivlik
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-[#0f172a] text-white rounded-[2rem] p-8 shadow-2xl relative overflow-hidden ring-1 ring-white/10 group cursor-default">
            <div className="relative z-10 space-y-6">
                <div className="w-14 h-14 bg-indigo-600/10 rounded-2xl flex items-center justify-center border border-white/10 group-hover:bg-indigo-600 transition-colors duration-500">
                  <TrendingUp className="text-indigo-400 group-hover:text-white" />
                </div>
                <h2 className="text-2xl font-black mb-4 tracking-tighter leading-tight text-white">AI Bilgi Bazası</h2>
                ...
                <div className="p-4 bg-white/5 rounded-2xl border border-white/5 backdrop-blur-md">
                    ...
                    <div className="text-sm font-black flex items-center gap-2">Gemini 2.0 Flash <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-ping" /></div>
                </div>
                <button className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl shadow-indigo-600/20 active:scale-95 transition-all">Məlumatları Yenilə</button>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[1000000] flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-[3rem] w-full max-w-xl shadow-2xl animate-in zoom-in-95 duration-300 my-auto overflow-hidden">
            <div className="p-8 sm:p-10 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
              ...
              <button onClick={() => setIsModalOpen(false)} className="p-3.5 hover:bg-slate-100 rounded-2xl transition-all text-slate-400"><X size={24} /></button>
            </div>
            <form onSubmit={handleCreateBot} className="p-8 sm:p-10 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <textarea required placeholder="AI necə danışmalıdır?" className="w-full p-4.5 rounded-2xl bg-slate-100 border-2 border-transparent focus:bg-white focus:border-indigo-600 outline-none transition-all h-32 resize-none font-bold text-slate-700 text-sm" value={formData.prompt} onChange={e => setFormData({...formData, prompt: e.target.value})} />
              </div>
              <div className="space-y-1.5">
              ...
              <button type="submit" disabled={submitting} className="w-full py-5 bg-indigo-600 text-white rounded-[1.5rem] font-black text-lg shadow-2xl shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-70 mt-4"> {submitting ? <Loader2 className="animate-spin" /> : "Botu Aktivləşdir 🚀"} </button>
            </form>
          </div>
        </div>
      )}
